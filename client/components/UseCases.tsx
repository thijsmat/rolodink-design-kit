      ...
      <div className="max-w-[1136px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          {useCases.map((useCase, index) => (
            <div key={index} className="bg-white border border-azure/10 rounded-lg sm:rounded-2xl p-4 sm:p-6 text-center">
              <div className="w-10 sm:w-12 h-10 sm:h-12 rounded-full bg-gold/10 flex items-center justify-center mx-auto mb-2 sm:mb-4">
                {useCase.icon}
              </div>
              <h3 className="font-semibold text-azure mb-1 sm:mb-2 text-sm sm:text-base">{useCase.title}</h3>
              <p className="text-xs sm:text-sm text-grey leading-relaxed">{useCase.description}</p>
            </div>
          ))}
        </div>
      </div>
